...
type STM interface {
	...
	// Del deletes a key.
	Del(key string)
	//GetKeys return all keys present in read and write sets
	GetKeys() []string
...
func mkSTM(c *v3.Client, opts *stmOptions) STM {
	...
	case RepeatableReads:
		s := &stm{client: c, ctx: opts.ctx, getOpts: []v3.OpOption{v3.WithPrefix(), v3.WithSerializable()}}
		s.conflicts = func() []v3.Cmp { return s.rset.cmps() }
		return s
	case ReadCommitted:
		s := &stm{client: c, ctx: opts.ctx, getOpts: []v3.OpOption{v3.WithPrefix(), v3.WithSerializable()}}
...
func (rs readSet) add(keys []string, txnresp *v3.TxnResponse) {
	for _, resp := range txnresp.Responses {
		...
		for _, kv := range getResp.Kvs {
			r := &v3.GetResponse{}
			r.Header = getResp.Header
			r.More = getResp.More
			r.Count = getResp.Count
			p := &mvccpb.KeyValue{}
			p.Key = kv.Key
			p.Value = kv.Value
			p.CreateRevision = kv.CreateRevision
			p.ModRevision = kv.ModRevision
			p.Version = kv.Version
			p.Lease = kv.Lease
			r.Kvs = append(r.Kvs, p)
			rs[string(kv.Key)] = r
		}
	}
}
...
func (s *stm) GetKeys() []string {
	keys := []string{}
	keysMap := make(map[string]bool)
	for key, _ := range s.rset {
		keysMap[key] = true
	}
