...
func (nuageetcd *NuageEtcdClient) Init(nkmConfig *config.NuageKubeMonConfig) error {
	...
	nuageetcd.clusterNetwork, err = api.IPv4SubnetFromString(nkmConfig.MasterConfig.NetworkConfig.ClusterNetworks[0].CIDR)
	if err != nil {
		...
		if noOfSubnets != 1 && ACTIVEIPCount*100 < noOfSubnets*nuageetcd.maxIPCount*SCALE_DOWN_THRESHOLD {
			for _, kv := range subnetResp.Kvs {
				if err := json.Unmarshal(kv.Value, snet); err != nil {
					...
					if 0 == snet.ACTIVEIP ||
						(snet.VSDID == deallocatedSubnet && 1 == snet.ACTIVEIP) {
						...
						if 1 == snet.ACTIVEIP {
							//if we are the deleting the subnet, then put of same is not required.
							ops = ops[1:]
						}
					}
				}
